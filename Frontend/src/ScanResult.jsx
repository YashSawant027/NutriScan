...
export default function ScanResult({ data, onClose }) {
  ...
  const { product_display_name, health_score = 0, eco_score = 0, ingredients = [], alerts = [], sustainability = {} } = ai_analysis;
  ...
  const getStatusTheme = (status) => {
    switch (status?.toLowerCase()) {
      ...
      case 'warning': return { bg: "bg-[#FFF5F5]", text: "text-[#9B1C1C]", border: "border-[#FEE2E2]", badge: "bg-[#E74C3C] text-white", dot: "bg-[#E74C3C]" };
        ...
        <div className="p-8 pb-4 flex justify-between items-start">
          <div className="flex gap-4 items-center">
             ...
             <div className="max-w-[240px]">
                ...
                <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">{data.brand} — {data.barcode}</p>
             </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm">
            ...
            <div className="text-center">
               <div className="relative w-20 h-20 mx-auto flex items-center justify-center">
                  <svg className="w-full h-full -rotate-90">
                    <circle cx="40" cy="40" r="35" stroke="#F1F5F9" strokeWidth="6" fill="none" />
                    <circle cx="40" cy="40" r="35" stroke="#FF9F1C" strokeWidth="6" fill="none" 
                      strokeDasharray={220} strokeDashoffset={220 - (220 * eco_score) / 100} strokeLinecap="round" />
                  </svg>
                ...
                return (
                  <div key={i} className={`p-5 rounded-[2rem] border ${theme.bg} ${theme.border} flex items-center justify-between shadow-sm transition-transform active:scale-95 cursor-pointer`}>
                    <div className="flex items-center gap-4">
                      ...
                      <div><h4 className={`font-bold text-sm ${theme.text}`}>{ing.name}</h4><p className="text-[10px] opacity-60 font-bold uppercase tracking-tighter">{ing.sub}</p></div>
                    </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {Object.entries(sustainability).map(([key, value]) => (
                <div key={key} className="bg-white p-4 rounded-2xl text-center border border-slate-100 shadow-sm">
                  ...
                  <span className={`text-[9px] font-black px-2 py-1 rounded border ${value === 'Low' || value === 'Recyclable' ? 'text-emerald-600 bg-emerald-50 border-emerald-100' : 'text-orange-600 bg-orange-50 border-orange-100'}`}>{value}</span>
                </div>
              ))}
            </div>
          </div>
